...
class ClientUserCreationForm(UserCreationForm):
    ...
    birthdate = forms.DateField(
        widget=DateInput(
            attrs={
                "min": datetime.date(dt.today().year - 100, 1, 1).__str__(),
    ...
    sex = forms.ChoiceField(
        ...
        choices=sex_choices.SEX_CHOICES,
        initial=sex_choices.NOTSET,
        label=_("Pohlaví"),
    )
    nationality = forms.ChoiceField(
        choices=nationality.CHOICES,
        initial=nationality.CZE,
        label=_("Státní příslušnost"),
    )
    psychologist_key = forms.UUIDField(
        label=_("Klíč psychologa"),
        error_messages={"invalid": "Zadejte validní klíč.", "required": "!"},
    )
    terms_accepted = forms.BooleanField(
        label=_("Souhlasím s účastí ve výzkumu a se zpracováním osobních údajů"),
        required=True,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            "terms_accepted",
            HTML(
                """<button type="button" class="btn btn-sm btn-secondary mb-3" data-bs-toggle="modal" 
                data-bs-target="#termsOfUseDetailModal">Zobrazit souhlas</button>"""
            ),
            Div(
                Submit("submit", "Registrovat se"),
    ...
    def clean_psychologist_key(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                _(
                    "Zadaný klíč nepatří žádnému psychologovi. \
    ...
    def save(self, commit=True):
        ...
        psychologist = PsychologistProfile.objects.get(
            personal_key__exact=data.get("psychologist_key")
        )
...
class ClientChangeForm(UserChangeBaseForm):
    ...
    psychologist_key = forms.UUIDField(
        label=_("Klíč psychologa"),
        help_text=_(
            "Klíč psychologa zadejte pouze v případě, že potřebujete změnit svého psychologa. Vaše responze vidí vždy pouze aktuální psycholog."
        ),
        error_messages={"invalid": "Zadejte validní klíč."},
        required=False,
    )
    fields = ["birthdate", "sex", "nationality", "psychologist_key"]
    ...
    def save(self, user):
        ...
        data = self.cleaned_data
        psychologist_key = data.get("psychologist_key")
        print("psychologist_key", psychologist_key)
        psychologist = None
        if psychologist_key:
            psychologist = PsychologistProfile.objects.get(
                personal_key__exact=data.get("psychologist_key")
        ...
        if psychologist:
            profile.psychologist = psychologist.user
        profile.save()
